...
function NewsCardList({ 
  ...
  const [visibleCount, setVisibleCount] = useState(isMainPage ? 3 : cards.length);
    ...
    <section className="news-cards">
      ...
      <div className="news-cards__grid">
        {renderedCards.map((card) => {
          const isSaved = savedArticles.some((savedItem) => savedItem.id === card.id);
          ...
          return (
            <NewsCard 
              ...
              page={page} 
              isSaved={isSaved}
              onCardAction={() => isMainPage ? onCardSave(card) : onCardDelete(card)}
            />
          );
        })}
      </div>
