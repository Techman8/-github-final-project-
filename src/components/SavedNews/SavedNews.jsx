...
export default function SavedNews({ 
  ...
  const getKeywordSummary = () => {
    ...
    const sortedKeywords = Object.keys(countMap).sort((a, b) => countMap[b] - countMap[a]);
    ...
    <div className="saved-news">
      ...
      <section className="saved-news__info-block">
        ...
        <h2 className="saved-news__title">
          ...
          {currentUser ? currentUser.name : "User"}, you have {savedArticles.length} saved article{savedArticles.length !== 1 ? 's' : ''}
        </h2>
        
        {savedArticles.length > 0 && (
          <p className="saved-news__keywords">
            By keywords: <span className="saved-news__keywords-bold">{getKeywordSummary()}</span>
          </p>
        )}
      </section>
      ...
      <main className="saved-news__content">
        {savedArticles.length > 0 ? (
          ...
          <div className="saved-news__empty-state">
            <p className="saved-news__empty-text">Your bookmarked collection is empty.</p>
          </div>
        )}
      </main>
    </div>
